...
namespace KMU.HisOrder.MVC.Areas.Statistic.Controllers
{
    ...
    public class MonthlyController : Controller
    {
        ...
        public MonthlyController(KMUContext context)
        {
            _context = context;
        }
        ...
        public ChartRelated YearsPatientsDataCollation(string parent, int year, List<string> deptList)
        {

            List<PatientCountIng> ptData = DeptPatients(deptList, year).OrderBy(c => c.YearMon).ToList();
        ...
        public List<PatientCountIng> DeptPatients(List<string> deptList, int year)
        {
